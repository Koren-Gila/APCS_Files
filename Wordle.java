 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Koren Gila
 *	@version	1.0
...
{ 
	...
	public String openFileAndChooseWord(String inFileName, String testWord)
	{
		...
		boolean allowed = inAllowedWordFile(testWord);	
		if(allowed)
		{
			result = testWord;
		}
	...
	public boolean inAllowedWordFile(String possibleWord)
	{
		...
		Scanner input = FileUtils.openToRead(WORDS5);
		while(input.hasNext())
		{
			if(possibleWord.equalsIgnoreCase(input.next()))
			{
				allowed = true;
			}
		}
	...
	public void processGuess ( )
	{
		...
		{
			JOptionPane pane = new JOptionPane(letters + " isn't allowed");
	...
	public void drawPanel ( )
	{
		...
		{
			for(int i = 0; i<wordGuess[row].length(); i++)
			{
				for(int j = 0; j<word.length(); j++)
				{
					if(wordGuess[row].charAt(i) == word.charAt(j))
					{
						if(j==i)
							keyBoardColors[(int)wordGuess[row].charAt(i)-'A'] = 3;
						else if(j!=i && keyBoardColors[(int)wordGuess[row].charAt(i)-'A']!=3)
						{
							keyBoardColors[(int)wordGuess[row].charAt(i)-'A'] = 2;
						}
					}
				}
				
				if(keyBoardColors[(int)wordGuess[row].charAt(i)-'A']==0)
				{
					keyBoardColors[(int)wordGuess[row].charAt(i)-'A']=1;
				}
			}	
			...
			for(int row = 0; row < 6; row++)
			{
				if(wordGuess[row].length()!=0)
					for(int col = 0; col < 5; col++)
					{
						...
						{
							if(wordGuess[row].charAt(col)==word.charAt(count) && count == col)
							{
								keyBoardColors[(int)word.charAt(count)-'A']=3;
							}
							else if(wordGuess[row].charAt(col)==word.charAt(count))
							{
								keyBoardColors[(int)word.charAt(count)-'A']=2;
							}
							
							if(count == word.length()-2 && keyBoardColors[(int)word.charAt(count)-'A']==0)
								keyBoardColors[(int)wordGuess[row].charAt(col)-'A']=1;
						}
						
						if(wordGuess[row].length() != 0)											//  THIS METHOD IS INCOMPLETE.
						{
							if(keyBoardColors[(int)wordGuess[row].charAt(col)-'A']==3)
								StdDraw.picture(209 + col * 68, 650 - row * 68, "letterFrameGreen.png");
							else if(keyBoardColors[(int)wordGuess[row].charAt(col)-'A']==2)
								StdDraw.picture(209 + col * 68, 650 - row * 68, "letterFrameYellow.png");
							else if(keyBoardColors[(int)wordGuess[row].charAt(col)-'A']==1)
		...
		for(int [] pair : Constants.KEYPLACEMENT)
		{
			...
			{
				int letterCount = Constants.KEYBOARD[place].charAt(0)-'A';
				String color = new String("keyBackground.png");
				if(keyBoardColors[letterCount]==3)
				{
					color = "keyBackgroundGreen.png";
				}
				else if(keyBoardColors[letterCount]==2)
				{
					color = "keyBackgroundYellow.png";
				}
				else if(keyBoardColors[letterCount]==1)
				{
					color = "keyBackgroundDarkGray.png";
				}
				else
				{
					color = "keyBackground.png";
				}
						
				
				StdDraw.picture(pair[0], pair[1], color);
			}
	...
	public void checkIfWonOrLost ( )
	{
		...
		{
			activeGame = false;
			JOptionPane pane = new JOptionPane(lastWord + " is not the word!  Press RESET to begin again");
			JDialog d = pane.createDialog(null, "DEFEAT!");
			d.setLocation(365,250);
			d.setVisible(true);
		}
